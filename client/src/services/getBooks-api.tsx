...
async function getPopularBooks() {
  const url =
    "https://api.nytimes.com/svc/books/v3/lists/current/hardcover-fiction.json?api-key=pjDx1UgcbMBBNQg7RGulL8yTvwUeGYgc";
...
async function getBookByRequest({ request, page = 1 }: IRequest) {
  ...
  const url = `${BASE_URL}?q=${request}&startIndex=${
    page * 20 + 1
  ...
  return response.ok
    ? await response.json()
    : Promise.reject(new Error(`No results on request ${request}`));
}
